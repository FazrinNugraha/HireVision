    ...
    with col3:
        ...
        if "lokasi_terpilih" not in st.session_state:
            st.session_state.lokasi_terpilih = (
                "Jakarta Selatan" if "Jakarta Selatan" in list_lokasi else list_lokasi[0]
            )
...
def _run_prediction(judul, kategori, lokasi, peng, pend, sert, resources):
    ...
    with st.spinner("Memproses Random Forest + penyesuaian profil..."):
        ...
        total_multiplier = m_peng * m_pend * m_sert
...
def _render_rincian_kalkulasi(res):
    ...
    with st.expander("📋 Lihat Rincian Kalkulasi Multiplier"):
        df = pd.DataFrame({
            "Faktor": [
                "Jabatan", "Kategori", "Lokasi",
                "Level Pengalaman", "Pendidikan Terakhir", "Sertifikasi",
                "Total Gabungan", "Gaji Basis ML", "Hasil Akhir",
            ],
            "Pilihan": [
                res["judul"], res["kategori"], res["lokasi"],
                res["pengalaman"], res["pendidikan"], res["sertifikasi"],
                "—",
                f"Rp {res['gaji_basis']:,}",
...
""", unsafe_allow_html=True)
    ...
    df_kota = (
        df_map.groupby("Lokasi_Clean")["Jumlah_Lowongan"].sum().reset_index()
    )
    df_kota["Harga_Kos_Estimasi"] = df_kota["Lokasi_Clean"].apply(predict_kos_price)
    df_kota = df_kota.sort_values(by="Jumlah_Lowongan", ascending=False).reset_index(drop=True)
...
def render():
    ...
    try:
        ...
        if "last_prediction" in st.session_state:
            ...
            has_battle_input = _render_salary_battle(res)
            if has_battle_input:
