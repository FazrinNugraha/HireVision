...
def load_map_data(filepath):
    ...
    df["lat"] = df["Lokasi_Clean"].apply(
        lambda x: COORDINATES.get(x, [-6.2000, 106.8166])[0]
    )
    df["lon"] = df["Lokasi_Clean"].apply(
        lambda x: COORDINATES.get(x, [-6.2000, 106.8166])[1]
    )
    return df
...
def chat_with_career_bot(client, user_message, chat_history, system_prompt_text=None):
    ...
    try:
        model = genai.GenerativeModel(
            "gemini-2.5-flash", system_instruction=system_prompt_text
        )
